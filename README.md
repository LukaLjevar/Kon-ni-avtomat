...
## Navodila za uporabo
Klonirajte si repozitorij. Z ukazom `dune build` zgradite datoteko `tekstovniVmesnik.exe`. Z ukazom `cd src` se premaknite v mapo `src` in poženite ukaz `.\tekstovniVmesnik.exe`. Tako boste lahko začeli uporabljati avtomat.  **TO DO**
