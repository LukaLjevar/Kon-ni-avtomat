...
## Navodila za uporabo
Klonirajte si repozitorij. Z ukazom `dune build` zgradite datoteko `tekstovniVmesnik.exe`. Z ukazom `cd src` se premaknite v mapo `src` in poženite ukaz `.\tekstovniVmesnik.exe`. Tako boste lahko začeli uporabljati avtomat. Začnete z vnosom podniza, ki ga iščete (niz1) **brez narekovajev**. Nato lahko izbirate med tremi možnostmi. 
